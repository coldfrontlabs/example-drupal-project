...
##### Begin Dropfort Build Config #####

$multisiteDir = basename(__DIR__);

// Generate the regex pattern for trusted domains.
$settings['trusted_host_patterns'] = [];
$trustedDomains = explode(',', getenv('DF_DRUPAL_DOMAINS'));
array_walk($trustedDomains, 'trim');

foreach ($trustedDomains as $trustedDomain) {
  // Escapce the URL pattern.
  $trustedPattern = str_replace('.', '\.', $trustedDomain);
  // Add it to the trusted patterns.
  $settings['trusted_host_patterns'][] = '^' . $trustedPattern .'$';
}

// Add the fallback pattern for internal references.
// @todo determine if we still need this.
$settings['trusted_host_patterns'][] = '^drupal$';

// Add the env hostname to the trusted patterns on any port.
// Required for liveness checks in Kubernetes.
$settings['trusted_host_patterns'][] = '^' . getenv('HOSTNAME') . '?(:\d+)*$';

// Finally, since the hostname isn't currently maintained in kubernetes,
// let's allow any hostname for now.
// @todo remove once hostname is retained properly.
$settings['trusted_host_patterns'][] = '^.*$';

// Use updated config directory setup for Drupal 8.8+
// Use the DF_DRUPAL_CONFIG_DIR env variable, otherwise fallback to the Drupal default.
$settings['config_sync_directory'] = getenv('DF_DRUPAL_SETTINGS_CONFIG_SYNC_DIRECTORY') ?: '../config/sync';

// Add a default hash salt.
$settings['hash_salt'] = getenv('DF_DRUPAL_SETTINGS_HASH_SALT');

// Prioritize the Dropfort Environment type over others.
$env_type = getenv('DF_ENV_TYPE') ?: getenv('PANTHEON_ENVIRONMENT') ?: getenv('AH_SITE_ENVIRONMENT') ?: 'dev';

$config['config_split.config_split.local']['status'] = FALSE;
$config['config_split.config_split.feature']['status'] = FALSE;
$config['config_split.config_split.dev']['status'] = FALSE;
$config['config_split.config_split.qa']['status'] = FALSE;
$config['config_split.config_split.stage']['status'] = FALSE;
$config['config_split.config_split.prod']['status'] = FALSE;
$config['config_split.config_split.training']['status'] = FALSE;
...
switch ($env_type) {
  ...
  case 'feat':
  case 'feature':
    $config['config_split.config_split.feature']['status'] = TRUE;
  break;
  case 'test':
  case 'qa':
  ...
  case 'train':
  case 'training':
    $config['config_split.config_split.training']['status'] = TRUE;
  break;
  case 'prod':
  case 'live':
  ...
  break;
  default:
    $env_type = filter_var(strtolower(preg_replace("/[^A-Za-z0-9 ]/", '', $env_type)), FILTER_SANITIZE_ENCODED);
    if (!empty($env_type)) {
      $config['config_split.config_split.' . strval($env_type)]['status'] = TRUE;
    }
}

// Load environment specific services.
$settings['container_yamls'][] = $app_root . '/../services/' . $multisiteDir . '/override.services.yml';
$settings['container_yamls'][] = $app_root . '/../services/' . $multisiteDir . '/' . $env_type . '.services.yml';

// Set a location for the private files directory.
$settings['file_private_path'] = getenv('DF_DRUPAL_SETTINGS_FILE_PRIVATE_PATH') ?: '/var/www/files/private';
$settings['file_public_path'] = $settings['file_assets_path'] = 'sites/' . $multisiteDir . '/files';

// Set a location for the tmp directory.
$settings['file_temp_path'] = getenv('DF_DRUPAL_SETTINGS_FILE_TEMP_PATH') ?: '/var/www/files/tmp';

// Load database credentials from env vars.
$databases['default']['default'] = [
  // Set the database name to "drupal" for the default site, otherwise use the
  // site's dir at the name.
  'database' => $multisiteDir == 'default' ? getenv('DF_DRUPAL_DB_NAME') : $multisiteDir,
  'username' => getenv('DF_DRUPAL_DB_USER'),
  'password' => getenv('DF_DRUPAL_DB_PASSWORD'),
  'host' => getenv('DF_DRUPAL_DB_HOST'),
  'port' => getenv('DF_DRUPAL_DB_PORT') ?: 3306,
  'driver' => getenv('DF_DRUPAL_DB_DRIVER') ?: 'mysql',
  'prefix' => '',
  'collation' => getenv('DF_DRUPAL_DB_COLLATION') ?: 'utf8mb4_general_ci',
];
...
// @todo remove once cluster speaks https everywhere.
// Required to make symfony isSecure() work correctly.
$settings['reverse_proxy_addresses'] = [$_SERVER['REMOTE_ADDR']];
$settings['reverse_proxy_trusted_headers'] = \Symfony\Component\HttpFoundation\Request::HEADER_X_FORWARDED_FOR | \Symfony\Component\HttpFoundation\Request::HEADER_X_FORWARDED_HOST | \Symfony\Component\HttpFoundation\Request::HEADER_X_FORWARDED_PORT | \Symfony\Component\HttpFoundation\Request::HEADER_X_FORWARDED_PROTO | \Symfony\Component\HttpFoundation\Request::HEADER_FORWARDED;

// Automatically managed the connection to the Solr backends based on
// Dropfort env variables.
...
if (getenv('DF_MEMCACHE_SERVER')) {
  $settings['memcache']['servers'] = [getenv('DF_MEMCACHE_SERVER') => 'default'];
  $settings['memcache']['bins'] = ['default' => 'default'];
  $settings['memcache']['key_prefix'] = $multisiteDir . '_' . md5(getenv('DF_DRUPAL_SETTINGS_HASH_SALT'));
}

// Based on autodection from the Acquia Memcache settings repo.
// @see https://github.com/acquia/memcache-settings/blob/main/memcache.settings.php
use Composer\Autoload\ClassLoader;

/**
 * Use memcache as cache backend if available.
 *
...
/**
 * Load env specific override configuration, if available.
 *
 * Use settings.ENV_TYPE.php to override variables on secondary (staging,
 * development, etc) installations or env specific versions of this site.
 *
 * Keep this code block at the end of this file to take full effect.
 */
if (file_exists($app_root . '/' . $site_path . '/settings.' . $env_type . '.php')) {
  include $app_root . '/' . $site_path . '/settings.' . $env_type .'.php';
}
