...
// Setup local dev trusted host pattern.
$dfProjectBaseUrl = getenv('PROJECT_BASE_URL') ?: 'dflocal.net';
$defaultSiteBaseUrl = 'drupal.' . $dfProjectBaseUrl;
$trustedDomains[] = $defaultSiteBaseUrl;
...
// Add multisite dir to the trusted patterns.
$multisiteDir = basename(__DIR__);
$settings['trusted_host_patterns'][] = '^' . $multisiteDir . '$';
...
$config_sync_dir = getenv('DF_DRUPAL_CONFIG_DIR') ?: '../config/sync';
$config_sync_folder = end(explode('/', $config_sync_dir));

if ($multisiteDir != 'default') {
  // BUild out where the config dir is.
  $config_sync_dir = $config_sync_dir . '/../' . $multisiteDir . '/' . $config_sync_folder;
}
...
$settings['hash_salt'] = 'somethinglongandfakethatdoesnotmatter';
...
$settings['file_private_path'] = getenv('WEB_DOCUMENT_ROOT') . '/sites/default/files/private';
...
$databases['default']['default'] = [
  ...
  'database' => $multisiteDir == 'default' ? getenv('DB_NAME') : $multisiteDir,
...
// Migration database connection information.
$databases['migrate']['default'] = $databases['default']['default'];
$databases['migrate']['default']['host'] = 'migrate';
...
// Memcache settings if required.
// Uncomment below to activate memcache caching.
$settings['memcache']['servers'] = ['memcache:11211' => 'default'];
$settings['memcache']['bins'] = ['default' => 'default'];
$settings['memcache']['key_prefix'] = $multisiteDir;
